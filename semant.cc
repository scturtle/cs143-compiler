...
ClassTable::ClassTable(Classes classes) : semant_errors(0), error_stream(cerr) {
    ...
    for (int i = classes->first(); classes->more(i); i = classes->next(i)) {
        ...
        if (name != SELF_TYPE && // SELF_TYPE cannot be redefined
            classMap.find(name) == classMap.end()) // class not defined
            classMap[name] = cls;
        else
            semant_error(cls) << "redefined class " << cls << endl;
    }

    // check Main class and main method
    if (classMap.find(Main) == classMap.end())
        semant_error() << "Class Main is not defined." << endl;
    else if (lookup(Main, main_meth) == nullptr)
        semant_error() << "no main method in Main class" << endl;
    ...
    for (auto &kv : classMap) {
        ...
        if (cls->get_name() == Object)
            continue;
        auto cur = cls->get_parent();
        while (cur != Object) {
            // parent cannot be self
            if (cur == SELF_TYPE)
                break;
            if (classMap.find(cur) == classMap.end()) {
                semant_error(cls) << cls->get_name()
                                  << " inherits from an undefined class " << cur
                                  << endl;
                break;
            } else if (cur == cls->get_name()) {
                char err[100];
                std::sprintf(err,
                             "Fatal Error: circle in %s's class hirerarchy",
                             cls->get_name()->get_string());
                fatal_error(err); // exit(1);
            } else {
                cur = classMap[cur]->get_parent();
            }
        }
    }
}
...
Symbol ClassTable::lub(const Symbol a, const Symbol b, const Symbol cls) {
    return lub(a == SELF_TYPE ? cls : a, b == SELF_TYPE ? cls : b);
}
////////////////////////////////////////////////////////////////////

Symbol loop_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    Symbol t = this->pred->tc(o, m, c);
    this->body->tc(o, m, c);
    if (t != Bool)
        m->semant_error(m->lookup(c)) << "loop pred" << endl;
    this->type = Object;
    return this->type;
}
...
Symbol typcase_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    for (int i = cases->first(); cases->more(i); i = cases->next(i)) {
        ...
        if (!m->leq(actural, decl, c))
            m->semant_error(m->lookup(c)) << "typcase not match" << endl;
        acts.insert(actural);
        if (decls.find(decl) == decls.end())
            decls.insert(decl);
        else
            m->semant_error(m->lookup(c)) << "typcase duplicate branch type"
                                          << endl;
    }
    // lub all
    Symbol t = nullptr;
    for (auto ct : acts)
        t = t == nullptr ? ct : m->lub(t, ct, c);
    this->type = t;
    return this->type;
}
...
Symbol assign_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    if (this->name == self) {
        m->semant_error(m->lookup(c)) << "Cannot assign to 'self'." << endl;
        return c;
    }
    Symbol newt = this->expr->tc(o, m, c);
    // if there is old type, check leq
    Symbol oldt = o->lookup(this->name);
    if (oldt != nullptr && !m->leq(newt, oldt, c))
        m->semant_error(m->lookup(c)) << "assign" << endl;
    this->type = newt;
    // o->addid(this->name, newt);
    return this->type;
}
...
Symbol new__class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (tn == SELF_TYPE)
        ...
        if (m->lookup(tn) != nullptr)
            ...
            m->semant_error(m->lookup(c)) << "new, cannot find class " << tn
                                          << endl;
            this->type = Object;
        }
    }
...
Symbol static_dispatch_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (!m->leq(t0, this->type_name, c)) {
        m->semant_error(m->lookup(c)) << "static_dispatch not match" << endl;
        t0 = this->type_name;
    }
    // parameter types
    ...
    if (meth == nullptr) {
        m->semant_error(m->lookup(c)) << "staticdispatch, cannot find method "
                                      << name << " of " << t0 << endl;
    ...
    auto it = pts.begin();
    for (int i = formals->first(); ok && formals->more(i);
         ++it, i = formals->next(i))
        ok = m->leq(*it, formals->nth(i)->get_type(), c);
    if (!ok) {
        m->semant_error(m->lookup(c)) << "staticispatch, class " << c
                                      << " method " << name
                                      << " parameters types not match" << endl;
        this->type = Object;
        return this->type;
    }
    // self_type in method's return type
    if (meth->get_type() == SELF_TYPE)
        this->type = t;
    else
...
Symbol dispatch_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (meth == nullptr) {
        m->semant_error(m->lookup(c)) << "dispatch, cannot find method " << name
                                      << " of " << t0 << endl;
        this->type = Object;
        return this->type;
    }
    ...
    for (int i = formals->first(); ok && formals->more(i);
        ...
        ok = m->leq(*it, formals->nth(i)->get_type(), c);
    if (!ok) {
        m->semant_error(m->lookup(c)) << "dispatch, class " << c << " method "
                                      << name << " parameters types not match"
                                      << endl;
        this->type = Object;
        return this->type;
    }
...
Symbol cond_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (tp != Bool)
        m->semant_error(m->lookup(c)) << "cond pred" << endl;
    this->type = m->lub(t1, t2, c);
    return this->type;
}
...
Symbol plus_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (t1 != Int || t2 != Int)
        m->semant_error(m->lookup(c)) << "plus" << endl;
    this->type = Int;
    return this->type;
}
...
Symbol sub_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    Symbol t2 = this->e2->tc(o, m, c);
    if (t1 != Int || t2 != Int)
        m->semant_error(m->lookup(c)) << "sub" << endl;
    this->type = Int;
    return this->type;
}
...
Symbol mul_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    Symbol t2 = this->e2->tc(o, m, c);
    if (t1 != Int || t2 != Int)
        m->semant_error(m->lookup(c)) << "mul" << endl;
    this->type = Int;
    return this->type;
}
...
Symbol divide_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    Symbol t2 = this->e2->tc(o, m, c);
    if (t1 != Int || t2 != Int)
        m->semant_error(m->lookup(c)) << "divide" << endl;
    this->type = Int;
    return this->type;
}
...
Symbol neg_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    Symbol t1 = this->e1->tc(o, m, c);
    if (t1 != Int)
        m->semant_error(m->lookup(c)) << "neg" << endl;
    this->type = Int;
    return this->type;
}
...
Symbol lt_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    Symbol t2 = this->e2->tc(o, m, c);
    if (t1 != Int || t2 != Int)
        m->semant_error(m->lookup(c)) << "lt" << endl;
    this->type = Bool;
    return this->type;
}
...
Symbol eq_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if ((t1 == Int && t2 != Int) || (t1 != Int && t2 == Int) ||
        ...
        m->semant_error(m->lookup(c)) << "eq" << endl;
    this->type = Bool;
    return this->type;
}
...
Symbol leq_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    Symbol t2 = this->e2->tc(o, m, c);
    if (t1 != Int || t2 != Int)
        m->semant_error(m->lookup(c)) << "leq" << endl;
    this->type = Bool;
    return this->type;
}
...
Symbol comp_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    Symbol t1 = this->e1->tc(o, m, c);
    if (t1 != Bool)
        m->semant_error(m->lookup(c)) << "comp" << endl;
    this->type = Bool;
    return this->type;
}
...
Symbol object_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    if (this->name == self) {
        this->type = SELF_TYPE;
    } else if (o->lookup(this->name) != nullptr) { // lookup in env
        this->type = o->lookup(this->name);
    } else { // lookup in inherits
        Symbol cls = c;
        Symbol t = nullptr;
        while (m->lookup(cls) != nullptr) {
            auto f = m->lookup(cls, this->name);
            if (f != nullptr && f->is_attr()) {
                ...
                if (cls == Object)
                    break;
                cls = m->lookup(cls)->get_parent();
            }
        }
        if (t == nullptr) {
            m->semant_error(m->lookup(c)) << "object " << this->name
                                          << " not found" << endl;
            this->type = Object;
        } else
            this->type = t;
...
Symbol method_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (!m->leq(t, return_type, c))
        m->semant_error(m->lookup(c))
            ...
            << " of method h does not conform to declared return type " << t
            << "." << endl;
    return return_type;
}
...
Symbol attr_class::tc(SymTable *o, ClassTableP m, Symbol c) {
    ...
    if (t == SELF_TYPE)
        return c;
    if (t != No_type && !(m->leq(t, type_decl, c))) // no init
        m->semant_error(m->lookup(c)) << "attr type not match" << endl;
    return type_decl;
}

void class__class::tc(SymTable *o, ClassTableP m) {
    // check bad inherit
    for (auto tp : std::vector<Symbol>{ SELF_TYPE, Bool, Int, Str })
        if (parent == tp)
            m->semant_error(this) << "Class " << name
                                  << " cannot inherit class " << tp << "."
                                  << endl;
    // check duplicates attr or method
    // std::set<Symbol> saw; // for checking duplicate feature
    o->enterscope();
    // check all features
    for (int i = features->first(); features->more(i); i = features->next(i)) {
        ...
        //     m->semant_error(this) << "redefined " << f->get_name() << endl;
        // saw.insert(f->get_name());
        if (f->get_name() == self) {
            m->semant_error(this) << "self cannot be name of attr or method"
                                  << endl;
        }
        // modify environment
        if (f->is_attr())
        ...
        // check method formals
            ...
            for (int i = fs->first(); fs->more(i); i = fs->next(i)) {
                Symbol fnm = fs->nth(i)->get_name();
                if (fs->nth(i)->get_type() == SELF_TYPE)
                    m->semant_error(this) << "Formal parameter " << fnm
                ...
                if (fnm == self)
                    m->semant_error(this)
                        ...
                        << endl;
                else if (saw_formal.find(fnm) != saw_formal.end())
                    m->semant_error(this) << "Formal parameter " << fnm
                                          << " is multiply defined." << endl;
                else
                    saw_formal.insert(fnm);
            }
        }
        // check override
        auto cur = parent;
        while (m->lookup(cur) != nullptr) {
            auto pf = m->lookup(cur, f->get_name());
            if (pf != nullptr && f->get_name() == pf->get_name()) {
                if (f->is_attr() && pf->is_method()) {
                    m->semant_error(this)
                        << "Attribute " << f->get_name()
                        << " is an method of an inherited class." << endl;
                    break;
                    ...
                    if (!len_ok) {
                        m->semant_error(this)
                            << "Incompatible number of formal parameters "
                               "in redefined method " << f->get_name() << endl;
                        break;
                    } else if (!ok) {
                        m->semant_error(this)
                            << "In redefined method " << f->get_name()
                            << " parameter type " << tnew
                            << " is different from original type " << told
                            << endl;
                        break;
                    }
                }
